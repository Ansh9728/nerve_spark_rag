...
try:
    from ollama import AsyncClient  # Ollama Python client (async)
except Exception as e:
    AsyncClient = None
    logger.warning(f"Could not import ollama.AsyncClient: {e}. Ollama features will be disabled.")
...
class RAGService:
    ...
    async def initialize(self):
        """Initialize the RAG service (vector store + Ollama client if available)"""
        self.vector_store = initialize_vector_store()
        
        # Initialize Ollama client if available
        if AsyncClient is None:
            logger.info("Ollama SDK not installed. Skipping Ollama initialization.")
            return

        # Host can be configured via settings; default to local Ollama port
        host = getattr(settings, "OLLAMA_HOST", "http://localhost:11434")
        try:
            self.ollama_client = AsyncClient(host=host)

            # Attempt to list models and choose the best available one
            preferred_models = [
                "smollm:135m",
                "phi:latest",
                "gemma3:latest",
            ...
            try:
                models_resp = await self.ollama_client.list()
                # models_resp may be a list of dicts or objects; normalize to names
                model_names = []
                if isinstance(models_resp, list):
                    for m in models_resp:
                        # m may be dict-like or object with 'name'/'model' attribute
                        name = None
                    ...
                    model_names = [str(models_resp).lower()]
                ...
                # If nothing matched, fall back to a safe default
                # self.model_name = chosen or getattr(settings, "OLLAMA_DEFAULT_MODEL", "gemma3")
                self.model_name = chosen or getattr(settings, "OLLAMA_DEFAULT_MODEL", None)

                if not self.model_name:
                    raise RuntimeError("No valid Ollama model selected. Please pull a model and set OLLAMA_DEFAULT_MODEL.")
                
                logger.info(f"Selected Ollama model: {self.model_name}")

            except Exception as e:
                # If listing models fails, fallback to configured default or gemma3
                self.model_name = getattr(settings, "OLLAMA_DEFAULT_MODEL", None)
                logger.warning(f"Could not list Ollama models, falling back to model '{self.model_name}': {e}")

        except Exception as e:
            logger.error(f"Failed to initialize Ollama client at {host}: {e}")
    ...
    async def generate_response(self, query: str, context: str) -> Dict[str, Any]:
        """Generate context-aware response using Ollama (chat)."""
        if not self.ollama_client or not self.model_name:
            return {
                "answer": "Ollama client not configured or no model selected. Please provide context manually.",
                "tokens": 0
            }
        
        logger.info(f"Ollama will call model: {self.model_name}")
        
        # Build the messages (system + user)
        # system_msg = {
        #     "role": "system",
        #     "content": "You are a helpful assistant for smart building management."
        # }
        # user_prompt = (
        #     "Use ONLY the provided context to answer the question.\n\n"
        #     "If the answer is not present in the context, respond with:\n"
        ...
        # 🔒 STRONG SYSTEM PROMPT (strict grounding)
        system_msg = {
            "role": "system",
            "content": (
                ...
                "6. If relevant, reference the source number like (Source 1).\n"
        ...
        if not self.model_name:
            raise RuntimeError("No valid Ollama model selected.")
        

        try:
            
            # Use chat API (non-streaming)
            ...
            # response shapes vary across versions: try multiple ways to extract text
            answer = None
            tokens = 0
            ...
            try:
                if isinstance(response, dict):
                    # e.g. response['message']['content'] or response.get('response')
                    answer = response.get('message', {}).get('content') or response.get('response')
                else:
                    # 2) try attribute access like response.message.content
            ...
            if not answer:
                try:
                    # sometimes response itself is a pydantic object with .message .content
                    answer = str(response)
                except Exception:
                    answer = "No answer returned from Ollama."

            # Note: Ollama responses may not provide token usage like OpenAI; set 0 when unknown
            return {
                "answer": answer,
                "tokens": tokens
            }
            ...
            logger.error(f"Error generating response (Ollama): {str(e)}")
            return {
                "answer": f"Error generating response: {str(e)}",
                "tokens": 0
            }
    ...
    async def query_with_context(self, query: str, top_k: int = 10, rerank_top_n:int = 5) -> RAGResponse:
        ...
        try:
            ...
            return RAGResponse(
                ...
                retrieval_time=retrieval_time,
                generation_time=generation_time,
                total_tokens=generation_result.get("tokens", 0),
                metrics=metrics
            )
